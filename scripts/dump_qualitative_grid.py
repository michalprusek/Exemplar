"""Dump predictions for the qualitative GRID figure.

For each dataset it fits the reported method (``METHOD``, best_v3 by default) on K=8 support masks
...
# The method whose predictions this figure shows. It was hardcoded to best_v2 while Table 1
# reports best_v3, which would put a figure and a table of two DIFFERENT methods in one paper --
# the defect the K-scaling figure actually shipped with (it plotted best_v2 and understated our
# own curve by up to 0.082). Defaults to the reported method; set QUAL_METHOD only to reproduce
# an older figure deliberately.
METHOD = os.environ.get("QUAL_METHOD",
                        "head_fusion_best_cgate_film_nobank_flat_es_ep500_wd4_do5_mix")
...
def main() -> None:
    ...
    # Print the METHOD. The first run of this script for the paper silently used the previous
    # configuration because the edit parameterising it had not reached the GPU box, and nothing
    # in the log or the .npz recorded which method produced the panels -- so the mismatch was
    # only catchable by reading the remote source. It is recorded now.
