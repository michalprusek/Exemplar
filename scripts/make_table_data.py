...
def dirs_for(method_key, ds, k):
    ...
    INSID3 used to run BOTH crf modes and be reported at the per-dataset better of the two, because
    the column was our own read-out and its refinement was ours to choose. Since 2026-09-02 it is
    the authors' released implementation in the single configuration their published anchor
    reproduces from (registry C88), so there is one directory and no steelman to apply.
    """
    if method_key in K_FREE:
        return [f"{FINAL}/{method_key}"]
    if method_key == "insid3":
        return [f"{FINAL}/insid3_k{k}"]
