...
def train_nnunet(shots, name, work, did, epochs, gpu, planner="ResEncUNetPlanner"):
    ...
    # PLANNER. nnU-Net's own startup banner says the default plans are superseded and points at the
    # ResEnc presets; running a baseline on the configuration its authors have deprecated is not the
    # "best documented configuration" the fairness protocol requires, and it matters here because
    # this baseline already beats us on most of the screen. ResEncUNetPlanner writes
    # nnUNetResEncUNetPlans, which the trainer then has to be pointed at with -p.
    plans = {"ResEncUNetPlanner": "nnUNetResEncUNetPlans",
             "ExperimentPlanner": "nnUNetPlans"}[planner]
    _run([os.path.join(sys.prefix, "bin", "nnUNetv2_plan_and_preprocess"),
          "-d", str(did), "-c", "2d", "-pl", planner, "--verify_dataset_integrity"], env)
    trainer = "nnUNetTrainer" if epochs == 1000 else f"nnUNetTrainer_{epochs}epochs"
    _run([os.path.join(sys.prefix, "bin", "nnUNetv2_train"),
          str(did), "2d", "all", "-tr", trainer, "-p", plans], env)

    model_dir = os.path.join(res, f"Dataset{did:03d}_{name}",
                             f"{trainer}__{plans}__2d")
    # torch.device("cuda", 0) here would predict on PHYSICAL GPU 0 whatever --gpu asked for:
    # CUDA_VISIBLE_DEVICES is set only in the subprocess env used for TRAINING, and this
    # predictor is built in the parent. With --gpu 1 that silently contends with whatever
    # campaign owns GPU 0, which is how a benchmark run starts OOMing a different job.
    dev = torch.device("cuda", gpu) if torch.cuda.is_available() else torch.device("cpu")
    pred = nnUNetPredictor(tile_step_size=0.5, use_gaussian=True, use_mirroring=True,
                           device=dev, allow_tqdm=False)
...
def main():
    ...
    ap.add_argument("--pool", type=int, default=None,
                    help="default: the campaign's per-dataset pool. A flat 20 would differ from\n"
                         "run_campaign for ctc_u373/isbi2012em/fisbe, and a different pool size\n"
                         "changes the support draw AND shifts the test slice on download-kind\n"
                         "datasets -- the exact trap the protocol section of CLAUDE.md warns of.")
    ...
    ap.add_argument("--planner", default="ResEncUNetPlanner",
                    choices=["ResEncUNetPlanner", "ExperimentPlanner"],
                    help="ResEnc is nnU-Net's current recommendation; its own banner calls the\n"
                         "default plans superseded. Running a baseline on a deprecated preset\n"
                         "is not the best documented configuration.")
    ...
    def _campaign_pool(ds):
        """Whatever run_campaign would use, read from it rather than duplicated here."""
        import run_campaign
        return run_campaign.pool_for(ds)

    for di, name in enumerate(args.datasets.split(",")):
        # RESUME: a dataset that already has a complete score record is skipped. Each record holds
        # all seeds and is written only after the last one, so its presence means the whole dataset
        # finished -- a paused run resumes by re-invoking this script and redoing only what is
        # missing, never re-training the datasets already on disk.
        done = score_record_path(args.score_dir, "nnunet", name)
        if os.path.exists(done):
            print(f"  [{name}] already complete ({done}) -- skipping", flush=True)
            continue
        spec = PANEL[name]
        ...
        pk = PRIMARY[eff]
        n_pool = args.pool if args.pool else _campaign_pool(name)
        pool, test = load_dataset(spec, n_pool, args.test, seed=0)
        ...
        for seed in range(args.seed_start, args.seed_start + args.seeds):
            ...
            predict = train_nnunet(shots, name, work, did, args.epochs, args.gpu, args.planner)
            scores = []
            for im, lb in test:
                fg = predict(im) > 0
        ...
        path = write_score_record(
            ...
            protocol={"support": args.support, "pool": n_pool, "test": len(test),
