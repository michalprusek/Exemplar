...
def main():
    ...
    p.add_argument("--method", default="head_fusion_best_nobank_flat_es_ep500_wd4_do5_mix",
    ...
    for ip, mp in sup_pairs:
        ...
        if m.ndim == 3:
            # Drop alpha BEFORE collapsing. An RGBA mask exported from ImageJ, GIMP or QuPath has
            # alpha=255 everywhere, so a max over all four channels marks the entire image as
            # foreground; the fit then succeeds on all-foreground supports and the quality drop
            # reads as the method being weak rather than as an input that never arrived.
            m = m[..., :3].max(-1)
        ...
        if fg.all() or not fg.any():
            raise SystemExit(f"{mp}: mask is {'entirely' if fg.all() else 'nowhere'} foreground. "
                             f"An all-foreground mask is usually an RGBA export read with its "
                             f"alpha channel; fitting on it produces a confident, useless model.")
        support.append(LabeledExample(im, enc.extract(im), fg.astype(int)))
