...
def make_backend(name: str, cfg, dev: str, refine: str = "none", enc=None, support_k: int = 0):
    ...
    if name == "correspondence":
        ...
        # Defaults for EVERY lever read at the constructor call below. They live here, before the
        # branch, because `make_backend` is the single path every benchmark takes and only
        # `head_fusion_best_*` names enter the token parser -- assigning them inside that branch
        # left `head_fusion_adaptive`, bare `head_fusion_best` and the AL loop raising
        # UnboundLocalError. These values are the reported configuration.
        dino_scale = "both"; proj_dim = 32; n_epochs = None; toks = ()
        stem = "wide"; hidden = 256; depth = 2; hidden_film = 64
        early_stop = False; wd = 0.0; do = 0.0; se = 0; mix = False; bil = 0
        # nnU-Net analogues. These MUST default here, outside the lever branch: the backend
        # constructor below is shared by every method name, so defining them only inside the
        # `head_fusion_best_` branch leaves them unbound for all the others -- which is exactly
        # what tests/test_ablation_arms.py caught.
        n_aug = 0; aug_elastic = 0.0; val_tile = 0.0; nnopt = False; val_loo = False; noaux = False
        if name.startswith("head_fusion_best_"):
            ...
            allowed = ("cgate", "corr", "film", "bank", "nobank", "mproto", "bdou", "prec", "nocls",
                       ...
                       "lean", "flat", "eca", "mix", "banknorm", "dih", "znorm", "taucal", "morphbank",
                       "noup", "bankonly")
            # Numeric-prefix rules below (h<N>, d<N>, f<N>, ep<N>, wd<N>, do<N>, se<N>, bil<N>,
            # pd<N>) are matched by startswith, so a NAMED token that happens to start with one of
            # those letters would be read as both. `d4` did exactly that -- it set depth=4 and the
            # head raised. Named tokens are therefore taken out first and only the remainder is
            # offered to the numeric rules, which makes the collision impossible rather than absent.
            _named = {"cgate", "corr", "film", "bank", "nobank", "mproto", "bdou", "prec", "nocls",
                      "coarseonly", "fineonly", "noloss", "nocolor", "rgbfeat", "bankselect",
                      "scaleconf", "lean", "flat", "eca", "mix", "banknorm", "dih", "es",
                      "taucal", "morphbank", "noup", "bankonly"}
            numeric = [t for t in toks if t not in _named]
            bad = [t for t in toks if t not in allowed
                   and not (t.startswith("pd") and t[2:].isdigit())
                   and not (t.startswith("h") and t[1:].isdigit())
                   and not (t.startswith("d") and t[1:].isdigit())
                   and not (t.startswith("f") and t[1:].isdigit())
                   and not (t.startswith("ep") and t[2:].isdigit())
                   and not (t.startswith("wd") and t[2:].isdigit())
                   and not (t.startswith("do") and t[2:].isdigit())
                   and not (t.startswith("se") and t[2:].isdigit())
                   and not (t.startswith("bil") and t[3:].isdigit())
                   # nnU-Net analogues, all OFF by default (see the block that reads them):
                   # naug<N> offline augmented copies, elas<N> elastic strength, vt<N> held-out
                   # strip as a percentage, nnopt nnU-Net's SGD+PolyLR.
                   and not (t.startswith("naug") and t[4:].isdigit())
                   and not (t.startswith("elas") and t[4:].isdigit())
                   and not (t.startswith("vt") and t[2:].isdigit())
                   and t not in ("es", "nnopt", "vloo", "noaux")]
            if bad:
            ...
            proj_dim = next((int(t[2:]) for t in numeric if t.startswith("pd") and t[2:].isdigit()), 32)
            # PARAMETER-BUDGET levers. `lean` makes the stem's first convolution 1x1 instead of 3x3,
            # which alone is 76% of the head's parameters; `h<N>` sets the stem width. Both default to
            # the reported configuration, so an unqualified method string is unaffected.
            stem = "flat" if "flat" in toks else "lean" if "lean" in toks else "wide"
            hidden = next((int(t[1:]) for t in numeric if t.startswith("h") and t[1:].isdigit()), 256)
            depth = next((int(t[1:]) for t in numeric if t.startswith("d") and t[1:].isdigit()), 2)
            hidden_film = next((int(t[1:]) for t in numeric if t.startswith("f") and t[1:].isdigit()), 64)
            # TRAINING BUDGET. 60 full-batch Adam steps is the reported figure and was never swept;
            # with no validation split it is unknown whether it under- or over-trains, and the two
            # possibilities predict opposite conclusions about head size. `_ep<N>` makes it testable.
            n_epochs = next((int(t[2:]) for t in numeric if t.startswith("ep") and t[2:].isdigit()), None)
            if n_epochs == 0:
                raise ValueError("ep0 is not a zero-training control: `n_epochs or default` reads "
                                 "0 as unset and silently gives the baseline budget. Use ep1.")
            # REGULARISATION, which has to arrive together with a longer budget: `es` runs the fit to
            # a training-loss plateau instead of truncating at a fixed step, and `wd<N>`/`do<N>` put
            # back explicitly what the truncation was doing implicitly. wd4 -> 1e-4, do5 -> 0.5.
            # nnU-NET ANALOGUES. What nnU-Net self-configures that this method does not, read out of
            # the installed nnunetv2 rather than paraphrased: a heavy augmentation pipeline, SGD with
            # Nesterov momentum 0.99 on a polynomial schedule, and validation every epoch. The
            # augmentation machinery already existed here (`n_aug` re-encodes warped copies through
            # the frozen backbone, which is the only correct way -- a warp does not commute with a
            # patch grid) but had no token and no A/B in the registry.
            n_aug = next((int(t[4:]) for t in numeric if t.startswith("naug") and t[4:].isdigit()), 0)
            aug_elastic = next((int(t[4:]) / 100.0 for t in numeric
                                if t.startswith("elas") and t[4:].isdigit()), 0.0)
            val_tile = next((int(t[2:]) / 100.0 for t in numeric
                             if t.startswith("vt") and t[2:].isdigit()), 0.0)
            nnopt = "nnopt" in toks
            val_loo = "vloo" in toks
            # ABLATION: drop the auxiliary outline head entirely. It is the last piece of
            # instance machinery in a semantic-only method, it has never been A/B'd (the
            # paper says so: "in every row and never ablated"), and with the adaptive-loss
            # constructor gone its BCE is applied UNGATED at weight 1.0 on every dataset.
            noaux = "noaux" in toks
            if aug_elastic > 0 and n_aug == 0:
                raise ValueError("elas<N> without naug<N> is a no-op: the elastic field is applied "
                                 "inside the offline augmenter, which only runs when n_aug > 0.")
            early_stop = "es" in toks
            _wdtok = next((int(t[2:]) for t in numeric if t.startswith("wd") and t[2:].isdigit()), None)
            if _wdtok == 0:
                raise ValueError("wd0 reads as 'no decay' but 10**-0 is 1.0, four orders past any "
                                 "sweep. Omit the token for no decay.")
            wd = 10.0 ** -_wdtok if _wdtok else 0.0
            do = next((int(t[2:]) / 10.0 for t in numeric if t.startswith("do") and t[2:].isdigit()), 0.0)
            # SE reduction ratio: se16 -> r=16 (132k params), se64 -> r=64 (34k), se128 -> r=128 (17k).
            se = next((int(t[2:]) for t in numeric if t.startswith("se") and t[2:].isdigit()), 0)
            # A token whose value IS the default yields an arm byte-identical to the baseline,
            # which is then written to its own score directory and read as an independent
            # configuration. Two registry rows differing only by seed noise, presented as an A/B.
            _noop = {"h256": hidden == 256, "d2": depth == 2, "f64": hidden_film == 64,
                     "se0": se == 0, "bil0": True, "do0": True}
            _dead = [t for t in toks if _noop.get(t)]
            if _dead:
                raise ValueError(f"token(s) {_dead} resolve to the default, so this arm is "
                                 f"IDENTICAL to the baseline; it would be logged as an A/B")
            # FUSION levers. `mix` = a nonlinear 1x1 after the concat (the guided_fuse block minus the
            # depthwise 3x3 the ladder measured as harmful). `bil<r>` = low-rank Hadamard interactions
            # between sources, which a concat + linear classifier cannot express at all.
            mix = "mix" in toks
            bil = next((int(t[3:]) for t in numeric if t.startswith("bil") and t[3:].isdigit()), 0)
        ...
        be = HeadFusionBackend(
            ...
            epochs=(n_epochs or (30 if al else 60)),
            n_aug=n_aug, aug_elastic=aug_elastic, val_tile=val_tile, val_loo=val_loo, sgd_poly=nnopt,
            # nnU-Net's own rate when its optimiser is selected; the head's 1e-3 is an Adam rate and
            # is two orders too small for SGD. Weight decay stays whatever wd<N> asked for.
            **({"lr": 1e-2} if nnopt else {}),
            ...
            boundary_head=((v2 or uni) and not dist and not noaux),  # W2 boundary head; `noaux` removes it
            ...
            bank_only=("bankonly" in toks),                          # ABLATION: zero DINO grids → bank-only head
            ...
            # ABLATION: `noup` forces plain bilinear in place of the prior-guided upsampler, so the
            # priors stop being the guidance signal and remain only evidence channels. None is the
            # backend default, i.e. exactly what the head does when no upsampler is supplied.
            upsampler=(None if "noup" in toks else sf_up.get(name)),   # learned upsampler (None=bilinear)
            stem=stem, hidden=hidden, depth=depth,         # parameter-budget levers (default = reported)
            eca=("eca" in toks),                           # soft per-image channel attention (5 params)
            early_stop=early_stop, weight_decay=wd, dropout=do, se=se, mix=mix, bilinear=bil,
            tau_cal=("taucal" in toks),                    # support-derived readout threshold
            morph_orient=("morphbank" in toks),            # +10 top-hat / orientation prior channels
            banknorm=("banknorm" in toks),                 # support-derived per-channel bank scale
            znorm=("znorm" in toks),                       # support-foreground input intensity z-score
            aug=("d4" if "dih" in toks else "flip"),       # dihedral vs the reported flip-only
            hidden_film=hidden_film,
        )
    else:
