...
class MorphOrientBank(nn.Module):
    """Two families this bank has never had: ORDER-STATISTIC morphology, and ORIENTATION.

    Every other family here is a differential or linear operator -- Hessian, Laplacian, gradient,
    Gabor, phase congruency. There is no rank filter anywhere, and the structure tensor's angle is
    computed and then thrown away (only its eigenvalues are kept). Both gaps line up with measured
    failures, which is the reason to close them rather than add more scales of what is already here.

    TOP-HAT (image minus its opening) is the classical detector for small bright structures on a
    varying background, and BLACK TOP-HAT its dark counterpart. That is literally the dense-nuclei
    and bacteria case -- the panel's worst dataset, where six controlled interventions each moved the
    score by at most 0.01. An opening also BREAKS thin connections, which is the bridging error that
    [[foreground-is-the-bottleneck]] measured as costing 2-3x the AP of error placed apart.

    ORIENTATION is emitted as (sin 2t, cos 2t) rather than t: the structure tensor's angle is defined
    modulo pi, so t itself is discontinuous at the wrap and a filter reading it would see an edge
    that is not there. The double-angle form is continuous everywhere. COHERENCE
    (l1-l2)/(l1+l2) says how strongly oriented the neighbourhood is, which separates a filament from
    a texture with the same eigenvalue magnitudes.

    Costs nothing extra to compute the orientation: the structure tensor is already formed here.
    """

    def __init__(self, tophat_sizes: Iterable[int] = (3, 7, 15),
                 orient_sigma: float = 0.004, median_size: int = 5):
        super().__init__()
        self.tophat_sizes = tuple(int(s) for s in tophat_sizes)
        self.orient_sigma = float(orient_sigma)
        self.median_size = int(median_size)

    @property
    def n_channels(self) -> int:
        # 2 per top-hat scale (white + black), 3 orientation (sin2t, cos2t, coherence), 1 median
        return 2 * len(self.tophat_sizes) + 3 + 1
    ...
    def forward(self, img: torch.Tensor) -> torch.Tensor:
        outs = []
        for k in self.tophat_sizes:
            opened = self._dilate(self._erode(img, k), k)
            closed = self._erode(self._dilate(img, k), k)
            outs.append(img - opened)          # white top-hat: small BRIGHT structure
            outs.append(closed - img)          # black top-hat: small DARK structure
        # Orientation from the structure tensor, at one scale (the angle is scale-stable where it
        # means anything; extra scales would mostly duplicate the eigenvalue channels already here).
        ref = float(min(img.shape[-2], img.shape[-1]))
        s = _resolve_sigma(self.orient_sigma, ref)
        b = gaussian_blur(img, s)
        Ix, Iy = _central_diff_x(b), _central_diff_y(b)
        Jxx = gaussian_blur(Ix * Ix, s)
        Jyy = gaussian_blur(Iy * Iy, s)
        Jxy = gaussian_blur(Ix * Iy, s)
        # sin/cos of the DOUBLE angle, straight from the tensor entries -- no atan2, so no wrap and
        # no branch: (Jxx-Jyy, 2Jxy) is the double-angle vector, normalised to unit length.
        dx, dy = Jxx - Jyy, 2.0 * Jxy
        norm = torch.sqrt(dx * dx + dy * dy + 1e-12)
        outs.append(dx / norm)
        outs.append(dy / norm)
        outs.append(norm / (Jxx + Jyy + 1e-12))          # coherence in [0,1]
        # Median residual: what a rank filter removes is exactly the speckle a linear blur smears.
        m = self.median_size
        med = F.pad(img, (m // 2,) * 4, mode="reflect").unfold(2, m, 1).unfold(3, m, 1)
        outs.append(img - med.reshape(*img.shape, -1).median(dim=-1).values)
        return torch.cat(outs, dim=1)
...
class HyperBank(nn.Module):
    ...
    def __init__(
        ...
        # NEW family, default OFF: order-statistic morphology + orientation. Off by default
        # because every run in flight and every score record on disk was produced without it.
        use_morph_orient: bool = False,
        tophat_sizes: Iterable[int] = (3, 7, 15),
    ):
        super().__init__()
        ...
        self.use_morph_orient = use_morph_orient
        self.morph_bank = MorphOrientBank(tophat_sizes) if use_morph_orient else None
        ...
        n_morph = self.morph_bank.n_channels if use_morph_orient else 0
        ...
        if use_morph_orient:
            # Same per-family GroupNorm every other family gets: top-hat residuals and a
            # coherence in [0,1] live on wildly different scales, and the fusion head shares
            # one ridge penalty across channels, so an unnormalised family would be
            # effectively regularised differently from the rest.
            self.gn_morph = nn.GroupNorm(num_groups=1, num_channels=n_morph, affine=False)

        n_total = (n_frangi + n_sauvola + n_struct + n_int + n_gradmag
                    + n_gabor + n_moments + n_scattering + n_lindeberg
                    + n_log + n_deth + n_pc + n_vignette + n_dog + n_morph)
    ...
    def feature_maps(self, img: torch.Tensor) -> torch.Tensor:
        ...
        if self.morph_bank is not None:
            parts.append(self.gn_morph(self.morph_bank(img)))
        return torch.cat(parts, dim=1)
