...
def write_score_record(score_dir: str, *, method: str, dataset: str, metric: str,
    ...
    # ATOMIC write: serialise to a per-process temp file in the SAME directory, then rename. A plain
    # `open(path, "w")` is not atomic, so two processes writing the same (score_dir, method, dataset)
    # at the same instant interleave and leave a truncated or mixed JSON that a generator would crash
    # on or, worse, silently misread. This is not hypothetical: on 2026-07-26 a helper run and the
    # main K-scaling driver computed `isbi2012em` at K=16 concurrently, because the driver's
    # resume-skip only tests for the file when its loop REACHES the dataset. The two writes happened
    # not to overlap; that was timing, not safety. os.replace is atomic within a filesystem, so the
    # reader now sees either the old record or the new one and never a partial one.
