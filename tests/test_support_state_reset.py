...
def test_bank_scale_is_re_derived_for_a_new_support_draw():
    """`banknorm` derives a per-channel divisor from the support. Carrying seed 0's divisor into
    seeds 1..9 would make every later draw normalise by the wrong constants and collapse the
    across-seed variance the error bars exist to show -- the leak class this file was written for."""
    from active_segmenter.segment.base import reset_backend_for_new_support
    from active_segmenter.segment.head_fusion_backend import HeadFusionBackend
    be = HeadFusionBackend(device="cpu", banknorm=True)
    be._bank_scale = "stale"
    reset_backend_for_new_support(be)
    assert be._bank_scale is None, "the support-derived bank scale survived a new draw"
