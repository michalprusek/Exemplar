...
> **Splits.** DRIVE, HRF and ISBI2012-EM have no canonical train/test split in this harness: both official halves are written into one flat directory and permuted with `default_rng(seed=0)`, so the reported 20-image DRIVE test split is 11 official-test plus 9 official-training images. The other eight datasets use their own split.
